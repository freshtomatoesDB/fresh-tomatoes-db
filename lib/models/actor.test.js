...
describe('fresh-tomatoes-db routes', () => {
  ...
  it('finds an actor by id', async () => {
    ...
    const createdActor = await Actor.insert({
  ...
  it('finds actor in a movie', async () => {
    ...
    const actors = await Actor.findByMovieId(stepBrothersMovie.id);
  ...
  it('deletes an actor with the delete method', async () => {
    ...
    const createdActorTwo = await Actor.insert({
      movieId: stepBrothersMovie.id,
      name: 'John C Reilly',
      oscar: true
    });

    const deletedActor = await Actor.delete(createdActor.id);
    expect(deletedActor).toEqual(createdActor);

    const foundActor = await Actor.findByActorId(createdActor.id);
    expect(foundActor).toBeNull();

    const actors = await Actor.findAllActors();
    expect(actors).toEqual.arrayContaining(createdActorTwo);
